    ...
    <div className="space-y-0">
      ...
      <div className="sticky top-0 z-10 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => { setImportMode('smart'); setImportData([]); setImportResult(null); setShowImportModal(true); }}
            ...
            title="Import new or update existing assets from CSV"
          >
      ...
      {/* ─── Smart Import/Update Modal ──────────────────────────────────── */}
      {showImportModal && (
        <Modal title="📥 Import / Update Assets" onClose={() => setShowImportModal(false)} wide>
          <div className="space-y-4">
            {/* Info box explaining smart logic */}
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 text-xs text-blue-700">
              <p className="font-semibold mb-1">Smart Import — based on Asset Number:</p>
              <ul className="list-disc pl-4 space-y-0.5">
                <li><span className="font-medium text-green-700">New Asset Number</span> → Creates new asset</li>
                <li><span className="font-medium text-amber-700">Existing Asset Number</span> → Updates that asset</li>
                <li><span className="font-medium text-red-700">Duplicate in CSV</span> → Skipped (first occurrence wins)</li>
              </ul>
            </div>
            ...
            <div className="bg-slate-50 border border-dashed border-slate-300 rounded-lg p-6 text-center">
              ...
              <p className="text-xs text-slate-400 mt-2">Max 500 rows. Asset Number is the key field.</p>
            </div>

            {importData.length > 0 && (
              <div className="space-y-3">
                <span className="text-sm font-medium text-slate-700">{importData.length} rows parsed — ready to process</span>
                  ...
                  <table className="w-full text-xs">
                    <thead className="bg-slate-50 sticky top-0">
                      <tr>
                        ...
                        <th className="px-2 py-1 text-left font-semibold text-blue-700">Asset Number</th>
                    ...
                    <tbody>
                      {importData.slice(0, 20).map((row, i) => (
                        <tr key={i} className="border-t border-slate-100">
                          ...
                          <td className="px-2 py-1 font-medium">{row.assetNumber || <span className="text-slate-400 italic">empty</span>}</td>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-4 gap-2">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-green-700">{importResult.created || 0}</div>
                    <div className="text-xs text-green-600">Created</div>
                  </div>
                  <div className="bg-amber-50 border border-amber-200 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-amber-700">{importResult.updated || 0}</div>
                    <div className="text-xs text-amber-600">Updated</div>
                  </div>
                  <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-slate-600">{importResult.skipped || 0}</div>
                    <div className="text-xs text-slate-500">Skipped</div>
                  </div>
                  <div className="bg-red-50 border border-red-200 rounded-lg p-2 text-center">
                    <div className="text-lg font-bold text-red-700">{importResult.errors?.length || 0}</div>
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody>
                        {importResult.details.map((d, i) => (
                          <tr key={i} className="border-t border-slate-100">
                            <td className="px-2 py-1 text-slate-400">{d.row}</td>
                            <td className="px-2 py-1 font-medium">{d.assetNumber || '-'}</td>
                            <td className="px-2 py-1">
                              <span className={`px-1.5 py-0.5 rounded text-xs font-medium ${
                                d.action === 'created' ? 'bg-green-100 text-green-700' :
                                d.action === 'updated' ? 'bg-amber-100 text-amber-700' :
                                'bg-slate-100 text-slate-600'
                              }`}>{d.action}{d.reason ? ` — ${d.reason}` : ''}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
                ...
                {importResult.errors?.length > 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-2">
                    ...
                    <ul className="text-xs text-red-600 max-h-20 overflow-auto space-y-0.5">
